...
public class AFilmixController : BaseOnlineController<AFilmixSettings>
{
    ...
    public AFilmixController() : base(ModInit.conf.AFilmix)
    {
    ...
    async public Task<ActionResult> Index(string title, string original_title, byte clarification, short year, int postid = 0, short t = -1, short? s = null, bool rjson = false, bool similar = false, string source = null, string id = null, string imdb_id = null, string tmdb_id = null, string kinopoisk_id = null, short? serial = null)
    {
        ...
        var oninvk = new AFilmixInvoke(
           init, host, "lite/a-filmix", httpHydra,
        ...
        if (postid == 0)
        {
            ...
            {
                if (source.Equals("a-filmix", StringComparison.OrdinalIgnoreCase))
                {
            ...
            if (postid == 0)
            {
                var search = await InvokeCacheResult(
                    $"a-filmix:search:{title}:{original_title}:{year}:{clarification}:{similar}:{imdb_id}:{tmdb_id}:{serial}",
                    40,
                ...
                if (search.Value.Count > 1 && !similar)
                {
                    ...
                    foreach (var si in search.Value)
                    {
                        ...
                        string destHref = !string.IsNullOrEmpty(si.href)
                            ? $"{host}/lite/a-filmix?source=a-filmix&id={HttpUtility.UrlEncode(si.href)}&title={enc_title}&original_title={enc_original_title}"
                            : $"{host}/lite/a-filmix?postid={si.id}&title={enc_title}&original_title={enc_original_title}";
    ...
    rhubFallback:
        ...
        {
            // Prefer stream_ref for quality resolution
            ...
            {
                // Try first stream's ref for quality info
    ...
    [HttpGet]
    [AllowAnonymous]
    [Route("lite/a-filmix/stream")]
    [Route("lite/afilmix/stream")]
    async public Task<ActionResult> Stream([FromQuery(Name = "ref")] string refData)
    {
        ...
        try
        {
            ...
            var streamInvoke = new AFilmixInvoke(
                init, host, "lite/a-filmix", httpHydra,
            ...
            // Redirect to first quality via HostStreamProxy (Lampa will proxy through Lampac)
