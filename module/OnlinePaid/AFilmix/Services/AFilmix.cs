...
public class AFilmixInvoke
{
    #region static
    static readonly Serilog.ILogger Log = Serilog.Log.ForContext<AFilmixInvoke>();
    ...
    public AFilmixInvoke(AFilmixSettings init, string host, string route, HttpHydra httpHydra, Func<string, string> onstreamfile, bool rjson = false)
    {
        this.init = init;
        apihost = init.host;
        this.route = route;
    ...
    async public Task<List<SearchResultItem>> Search(string title, string original_title, byte clarification, short year, bool similar, string imdb_id = null, string tmdb_id = null, string kinopoisk_id = null, short? serial = null)
    {
        ...
        var uri = new StringBuilder($"{apihost}/search?source=filmix&sources=filmix");
        ...
        if (!string.IsNullOrEmpty(init.token))
            uri.Append($"&filmix_token={init.token}&filmix_device_id={init.device_id}");
    ...
    async public Task<ContentData> GetContent(int postid, string href = null)
    {
        ...
        {
            if (!string.IsNullOrEmpty(init.token))
                refJson = $"{{\"href\":\"{href}\",\"token\":\"{init.token}\",\"device_id\":\"{init.device_id}\"}}";
            else
                refJson = $"{{\"href\":\"{href}\"}}";
        }
        else if (!string.IsNullOrEmpty(init.token))
            refJson = $"{{\"id\":{postid},\"tmdb_id\":{postid},\"token\":\"{init.token}\",\"device_id\":\"{init.device_id}\"}}";
        else
            refJson = $"{{\"id\":{postid},\"tmdb_id\":{postid}}}";

        string body = $"{{\"source\":\"filmix\",\"ref\":{refJson},\"full\":true}}";
    ...
    {
        // Add token/device_id to ref for filmix auth
        JObject refWithAuth;
        if (!string.IsNullOrEmpty(init.token) && streamRef["token"] == null)
        {
            refWithAuth = new JObject(streamRef);
            refWithAuth["token"] = init.token;
            refWithAuth["device_id"] = init.device_id;
        }
        else
        {
            refWithAuth = streamRef;
        }

        string refStr = refWithAuth.ToString(Formatting.None);
        string body = $"{{\"source\":\"filmix\",\"ref\":{refStr}}}";
    ...
    public ITplResult Tpl(ContentData data, int postid, string title, string original_title, short t, short? s, StreamResponse streamResp = null)
    {
        ...
        else
        {
            ...
            if (s == null)
            {
                ...
                foreach (var season in distinctSeasons)
                {
                    tpl.Append(
                        ...
                        $"{host}{route}?rjson={rjson}&postid={postid}&title={enc_title}&original_title={enc_original_title}&s={season.title ?? season.season?.ToString()}",
            ...
            else
            {
                ...
                {
                    string voiceLink = host + $"{route}?rjson={rjson}&postid={postid}&title={enc_title}&original_title={enc_original_title}&s={s}&t={indexTranslate}";
                ...
                foreach (var episode in episodes)
                {
                    ...
                    // Extract quality from episode URL: _1080.mp4 → 1080p
                    var sq = new StreamQualityTpl();
                    string episodeUrl = episode.ref_data?["url"]?.ToString();
                    if (!string.IsNullOrEmpty(episodeUrl))
                    {
                        var qualityMatch = Regex.Match(episodeUrl, @"_(\d+)\.mp4");
                        if (qualityMatch.Success)
                            sq.Append(streamLink, $"{qualityMatch.Groups[1].Value}p");
                    }
                    if (sq.IsEmpty)
                        sq.Append(streamLink, "auto");
